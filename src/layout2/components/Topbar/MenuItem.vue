...
const hasOneShowingChild = (children: RouteOption[] = [], parent: RouteOption) => {
  ...
  if (showingChildren.length === 1) {
    onlyOneChild.value = {
      meta: showingChildren[0].meta,
      path: parent.path + showingChildren[0].path,
    }
    return true
  }
  ...
  <template v-if="!item.hidden">
    <el-menu-item v-if="hasOneShowingChild(item.children || [], item)" :index="resolvePath(onlyOneChild.path)">
      <el-icon v-if="onlyOneChild.meta && onlyOneChild.meta.icon !== '#'">
        <icon-park :type="onlyOneChild.meta.icon" theme="filled" />
      </el-icon>
    ...
    <el-sub-menu v-else :index="resolvePath(item.path)">
      <template v-if="item.meta" #title>
        <el-icon v-if="item.meta && item.meta?.icon !== '#'">
          <icon-park :type="item.meta.icon" theme="filled" />
        </el-icon>
        <span class="menu-title">{{ item.meta?.title }}</span>
      </template>
      <MenuItem v-for="child in item.children" :key="child.path" :item="child" :base-path="item.path" />
    </el-sub-menu>
  </template>
</template>
